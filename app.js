  ...
  syncPromise=(async()=>{
    ...
    while(tokenAtStart===sessionToken&&epochAtStart===sessionEpoch){
      ...
      try{
        ...
        lastSyncError=err;
        if(sessionError(err)&&tokenAtStart===sessionToken){sessionEpoch++;clearInterval(heartbeatTimer);localStorage.removeItem(PENDING_KEY);localStorage.removeItem(SESSION_KEY);sessionToken="";view="start";render("Tu sesión terminó. Recupérala con tu palabra clave.");}
        return false;
      }
    }
    return false;
...
async function load() {
  ...
  try{
    ...
    if(tokenAtStart!==sessionToken||epochAtStart!==sessionEpoch)return;
    if(sessionError(err)){sessionEpoch++;localStorage.removeItem(PENDING_KEY);localStorage.removeItem(SESSION_KEY);sessionToken="";view="start";render("Tu sesión terminó. Recupérala con tu palabra clave.");return;}
...
function renderStart(message) { const recovering=accessMode==="recover"; app.innerHTML=`<main class="universo start-view">${nav()}<section class="hero access-hero"><p class="tag">Bienvenido/a a la Guía de la Experiencia</p><h1>Tu viaje empieza<br><em>en este universo.</em></h1><p class="intro">${recovering?"Recupera tu recorrido ingresando únicamente tu palabra clave.":"Crea un acceso con tu nombre y una palabra clave."}</p><form class="access-form" onsubmit="loginParticipant(event)"><fieldset class="access-mode"><legend>Selecciona una opción</legend><label><input type="radio" name="access-mode" value="register" ${recovering?"":"checked"} onchange="updateAccessMode()"> Crear un viaje</label><label><input type="radio" name="access-mode" value="recover" ${recovering?"checked":""} onchange="updateAccessMode()"> Recuperar mi sesión</label></fieldset>${recovering?"":`<label class="field">Nombre completo<input id="name" value="${safe(trip.name)}" placeholder="Tu nombre y apellidos" autocomplete="name" minlength="2" maxlength="80" required></label>`}<label class="field">Palabra clave<input id="access-key" type="password" placeholder="Mínimo 10 caracteres" autocomplete="${recovering?"current-password":"new-password"}" minlength="10" maxlength="64" aria-describedby="access-key-notice" required></label><button class="primary" id="access-submit" type="submit">${recovering?"Recuperar mi viaje":"Crear mi viaje"} →</button><p class="access-key-notice" id="access-key-notice" role="note">${recovering?"Ingresa la palabra clave que guardaste al crear tu viaje.":"<strong>Guarda tu palabra clave en un lugar seguro.</strong> La necesitarás para recuperar la sesión. No podremos mostrártela ni enviártela después."}</p></form>${error(message)}<small>Usa una frase difícil de adivinar, de al menos 10 caracteres, y no reutilices una contraseña personal o corporativa.</small><a class="admin-entry" href="admin.html">Acceso administrador</a></section></main>`; }
function updateAccessMode(){const recover=document.querySelector('input[name="access-mode"]:checked')?.value==="recover",name=document.querySelector("#name")?.value.trim();if(name)trip.name=name;accessMode=recover?"recover":"register";render();}
...
async function loginParticipant(event){
  ...
  const mode=document.querySelector('input[name="access-mode"]:checked')?.value||"register",name=mode==="register"?(document.querySelector("#name")?.value.trim()||""):"",accessKey=document.querySelector("#access-key")?.value||"";
  accessMode=mode;if(mode==="register")trip.name=name;
  if(mode==="register"&&name.length<2)return render("Escribe tu nombre completo.");
  if(mode==="register"&&(name.length>80||/[\u0000-\u001f\u007f]/.test(name)))return render("El nombre debe tener entre 2 y 80 caracteres y no contener caracteres de control.");
  ...
  try{
    const {data,error:err}=await rpcWithRetry("universo_ingresar",{p_nombre:mode==="register"?name:null,p_palabra_clave:accessKey,p_modo:mode,p_legacy_client_id:clientId},1);
