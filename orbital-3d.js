...
(function () {
  ...
  function build(realm,progress){
    ...
    stage.innerHTML=`<div class="cosmos-galaxy-caption" aria-hidden="true">Galaxia empresa</div>
      ...
      <svg class="cosmos-constellation-lines" aria-hidden="true"><defs><clipPath id="constellation-viewport"><rect/></clipPath><linearGradient id="constellation-light"><stop stop-color="#8ca3b8" stop-opacity=".48"/><stop offset=".52" stop-color="#f5f9ff" stop-opacity=".92"/><stop offset="1" stop-color="#91a9bd" stop-opacity=".42"/></linearGradient></defs><g clip-path="url(#constellation-viewport)"><path class="secondary-constellations" fill="none" stroke="url(#constellation-light)" opacity=".28"/><path class="reference-constellation" fill="none" stroke="url(#constellation-light)"/></g></svg><div class="cosmos-labels"></div>`;
    ...
    function orbit(parent,radius,period,phase,tilt=0,color=0x678cb2,eccentricity=0,showGuide=true,orientation={}){
      ...
      const entry={anchor,plane,parent,radius,period,phase,eccentricity,line,body:null,frontDrop:orientation.frontDrop||0};moving.push(entry);return entry;
    }
    function satellite(){
    ...
    const primary=star(5.35,0xff4f0a,true);layer(primary,0);primary.traverse(o=>{o.layers.enable(6);o.renderOrder=20;const list=Array.isArray(o.material)?o.material:[o.material];list.filter(Boolean).forEach(material=>{material.depthTest=false;});});primary.userData.foregroundProtected=true;primaryOrbit.anchor.add(primary);
    ...
    const COMET_FIRST=8,COMET_PERIOD=52,COMET_DURATION=12.5;
    const cometRoutes=[
      u=>({position:new T.Vector3(-68+136*u,3.4+7.2*Math.sin(Math.PI*u),-17+34*u),tangent:new T.Vector3(136,Math.PI*7.2*Math.cos(Math.PI*u),34)}),
      u=>({position:new T.Vector3(68-136*u,-1.8+8.4*Math.sin(Math.PI*u),16-29*u+2.2*Math.sin(TAU*u)),tangent:new T.Vector3(-136,Math.PI*8.4*Math.cos(Math.PI*u),-29+TAU*2.2*Math.cos(TAU*u))}),
      u=>({position:new T.Vector3(-68+136*u,8.2-12.5*u+3.8*Math.sin(Math.PI*u),21-40*u),tangent:new T.Vector3(136,-12.5+Math.PI*3.8*Math.cos(Math.PI*u),-40)}),
      u=>({position:new T.Vector3(68-136*u,-5.2+10.4*u+3.2*Math.sin(TAU*u),-21+42*u),tangent:new T.Vector3(-136,10.4+TAU*3.2*Math.cos(TAU*u),42)})
    ];
    ...
    // The supplied reference remains the protagonist. Smaller, dimmer
    // asterisms provide depth around it without labels or visual competition.
    const zodiacPatterns=[
      ...
      {id:'background-a',center:[-7.4,4.7],scale:.58,points:[[-2,-.4],[-.8,.5],[.4,.1],[1.7,1],[2.4,.2]],edges:[[0,1],[1,2],[2,3],[3,4]]},
      {id:'background-b',center:[-6.5,-3.5],scale:.66,points:[[-1.8,.8],[-.7,-.2],[.5,.5],[1.6,-.7],[2.4,.2]],edges:[[0,1],[1,2],[2,3],[3,4],[1,3]]},
      {id:'background-c',center:[-.8,5.6],scale:.52,points:[[-2,-.1],[-.7,.7],[.3,-.2],[1.4,.5],[2.2,-.4]],edges:[[0,1],[1,2],[2,3],[3,4]]},
      {id:'background-d',center:[-.4,-6.2],scale:.48,points:[[-2,.5],[-.9,-.5],[.2,.3],[1.2,-.4],[2.1,.6]],edges:[[0,1],[1,2],[2,3],[3,4]]}
    ];
    ...
    for(let i=0;i<720;i++){
      ...
      skyPositions.push(x,y,z);const c=new T.Color([0xddeaff,0xf5f7ff,0xaec8dc,0xc8b8d8,0xd8cfb5][i%5]).multiplyScalar(.42+rng()*.48);skyColors.push(c.r,c.g,c.b);
    }
    ...
    [[-7.4,2.8,8.5,4.3,0x315dff,.22],[-4.2,-2.4,10.5,5.1,0x7137d4,.20],[-.7,3.2,9.2,4.4,0x257fbd,.18],[2.4,-3.5,10.8,5.2,0x713ca8,.20],[5.8,2.2,8.6,4.1,0x168f82,.17],[8.6,-3.5,6.8,3.8,0x435fd4,.16]].forEach(([x,y,w,h,color,opacity],i)=>{
    ...
    const dustPositions=[],dustColors=[],dustPalette=[0x607cff,0x8b4fd8,0x39a58d];
    for(let i=0;i<260;i++){
      const x=(rng()-.5)*23,y=(rng()-.5)*12,z=-12-rng()*8,density=.25+.75*rng();dustPositions.push(x,y,z);
      const c=new T.Color(dustPalette[i%dustPalette.length]).multiplyScalar(.34+density*.42);dustColors.push(c.r,c.g,c.b);
    }
    const dustGeometry=new T.BufferGeometry();dustGeometry.setAttribute('position',new T.Float32BufferAttribute(dustPositions,3));dustGeometry.setAttribute('color',new T.Float32BufferAttribute(dustColors,3));
    const dustMaterial=new T.PointsMaterial({map:glow,size:2.1,vertexColors:true,transparent:true,opacity:.31,alphaTest:.015,blending:T.AdditiveBlending,depthWrite:false,sizeAttenuation:false});materials.push(dustMaterial);
    ...
    function updatePositions(){
      moving.forEach(o=>{const distributed=o.distribution,mean=((distributed?distributed.slot+elapsed*TAU/distributed.period+Math.sin(elapsed*TAU/distributed.wobblePeriod+distributed.wobblePhase)*distributed.wobble:o.phase+elapsed*TAU/o.period)%TAU+TAU)%TAU;let a=mean;
        ...
        const x=o.radius*(Math.cos(a)-o.eccentricity),z=o.radius*Math.sqrt(1-o.eccentricity*o.eccentricity)*Math.sin(a);let y=0;
        if(o.frontDrop){o.plane.getWorldQuaternion(orbitQuaternion).invert();orbitCameraLocal.copy(cameraOffset).applyQuaternion(orbitQuaternion).normalize();const front=Math.max(0,(x*orbitCameraLocal.x+z*orbitCameraLocal.z)/o.radius);y=-o.frontDrop*front;}
        o.anchor.position.set(x,y,z);});
    ...
    function draw(dt=0){
      ...
      ['galaxy','system','launch','constellation'].forEach(name=>{const r=views[name];if(!r.visible||r.h<=0)return;moving.forEach(o=>{if(o.line)o.line.material.uniforms.uViewport.value.set(r.w,r.h);});renderer.setViewport(r.x,height-r.y-r.h,r.w,r.h);renderer.setScissor(r.x,height-r.y-r.h,r.w,r.h);renderer.clearDepth();
        if(name==='system'){
          cameras.system.layers.set(2);renderer.render(scene,cameras.system);renderer.clearDepth();cameras.system.layers.set(6);renderer.render(scene,cameras.system);cameras.system.layers.set(2);cameras.system.layers.enable(6);
        }else renderer.render(scene,cameras[name]);
      });renderer.setScissorTest(false);
      const bodyDisks=[...systemRecords(),...(!focusPlanet?[launchRecord]:[])].map(record=>{
        const view=record.view||'system',p=project(record.object,view),camera=cameras[view],defaultRadius=bodyExtent(record)*MAX_HOVER_SCALE*views[view].w/(camera.right-camera.left);
        if(record.id==='observatory'){const bounds=visualBounds(record,view);return{record,view,x:(bounds.left+bounds.right)/2,y:(bounds.top+bounds.bottom)/2,radius:Math.max(bounds.right-bounds.left,bounds.bottom-bounds.top)/2};}
        return{record,view,x:p.x,y:p.y,radius:defaultRadius};
      });
      ...
      const constellationPath=zodiacSvg.querySelector('.reference-constellation'),secondaryPath=zodiacSvg.querySelector('.secondary-constellations');
      const edgePath=hero=>guideEdges.filter(([, ,group])=>(group==='reference')===hero).map(([a,b])=>`M${pts[a].x.toFixed(2)},${pts[a].y.toFixed(2)} L${pts[b].x.toFixed(2)},${pts[b].y.toFixed(2)}`).join(' ');
      constellationPath.setAttribute('d',cv.visible?edgePath(true):'');constellationPath.setAttribute('stroke-width',String((.68+constellation.hoverAmount*.22)*displayScale));constellationPath.style.filter=`drop-shadow(0 0 ${1.1+2*constellation.hoverAmount}px #dcecff)`;
      secondaryPath.setAttribute('d',cv.visible?edgePath(false):'');secondaryPath.setAttribute('stroke-width',String(.42*displayScale));secondaryPath.style.filter='drop-shadow(0 0 1px #a9c9e8)';
    }
    ...
    function auditVisibility(){
      ...
      return {view:focusPlanet?'detail':'system',safeRect:{...views.system.safeRect},objects:visibleRecords.map(r=>{
        ...
        const p=project(r.object,name),radius=bodyExtent(r)*(1+(MAX_HOVER_SCALE-1)*r.hoverAmount)*pixelsPerUnit,sphereBounds={left:p.x-radius,right:p.x+radius,top:p.y-radius,bottom:p.y+radius},bounds=r.id==='observatory'?visualBounds(r,name):sphereBounds,limit=r.id==='observatory'?{left:view.x,right:view.x+view.w,top:view.y,bottom:view.y+view.h}:safe;
        const inside=bounds.left>=limit.left-.01&&bounds.right<=limit.right+.01&&bounds.top>=limit.top-.01&&bounds.bottom<=limit.bottom+.01;
        let rendered=false;r.visual.traverse(o=>{if(o.isMesh&&!o.material.transparent&&o.layers.test(camera.layers))rendered=true;});
        return{id:r.id,kind:r.kind,view:name,visible:view.visible,safeRect:{...limit},x:p.x,y:p.y,depth:p.z,pixelRadius:radius,maxHoverRadius:bodyExtent(r)*MAX_HOVER_SCALE*pixelsPerUnit,bounds,visualBounds:visualBounds(r,name),inside,selectable:view.visible&&inside&&rendered&&p.z>=-1&&p.z<=1};
